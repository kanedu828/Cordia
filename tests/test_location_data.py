...
class TestLocationData(unittest.TestCase):
    ...
    def test_locations_ordered_by_level(self):
        ...
        self.assertEqual(
            levels, sorted(levels), "Locations are not ordered by level_unlock"
        )
    ...
    def test_location_key_is_snake_case_and_name_is_capitalized(self):
        ...
        for location, data in location_data.items():
            with self.subTest(location=location):
                ...
                self.assertRegex(
                    location, snake_case_pattern, f"{location} is not in snake_case"
                )
                ...
                self.assertNotIn(
                    "_",
                    data.name,
                    f"{data.name} contains underscores, which is not allowed in names",
                )
                ...
                for word in words:
                    self.assertTrue(
                        word[0].isupper(),
                        f"'{word}' in '{data.name}' is not capitalized properly",
                    )
