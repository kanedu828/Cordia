        ...
        for monster_key, monster in {**monster_data, **boss_data}.items():
            with self.subTest(monster=monster_key):
                for gear_item, _ in monster.gear_loot:
                    self.assertIn(gear_item, gear_data, f"{gear_item} from {monster_key} is missing in gear_data")
    ...
    def test_monster_name_is_title_case(self):
        ...
        for monster_key, monster in {**monster_data, **boss_data}.items():
            with self.subTest(monster=monster_key):
                ...
                self.assertEqual(monster.name, expected_name, f"{monster_key}'s name should be {expected_name}, but got {monster.name}")
        ...
        for monster_key, monster in {**monster_data, **boss_data}.items():
            with self.subTest(monster=monster_key):
                ...
                if hasattr(monster, 'defense'):
                    self.assertLessEqual(monster.defense, 100, f"{monster_key} defense is greater than 100")
                
                # Check if resistance exists and is <= 100
                if hasattr(monster, 'resistance'):
                    self.assertLessEqual(monster.resistance, 100, f"{monster_key} resistance is greater than 100")
    ...
    def test_boss_monsters_are_of_type_boss(self):
        ...
        for boss_key, boss in boss_data.items():
            with self.subTest(boss=boss_key):
                self.assertEqual(boss.type, MonsterType.BOSS, f"{boss.name} should be of type BOSS but has {boss.type}")
