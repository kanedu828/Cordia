...
class LeaderboardService:
    def __init__(self, player_dao: PlayerDao, daily_leaderboard_dao: DailyLeaderboardDao, bot):
        ...
        self.scheduler.add_job(self.award_trophies_to_top_players, 'cron', hour=23, minute=55)
    ...
    async def upsert_daily_leaderboard(self, discord_id: int, exp: int, gold: int, monsters_killed: int):
        await self.daily_leaderboard_dao.upsert_daily_leaderboard(discord_id, exp, gold, monsters_killed)

    async def get_top_100_daily_players_by_column(self, column: str) -> list[DailyLeaderboard]:
        return await self.daily_leaderboard_dao.get_top_100_daily_players_by_column(column)
    
    async def get_player_daily_rank_by_column(self, discord_id: int, column: str) -> int:
        return await self.daily_leaderboard_dao.get_player_daily_rank_by_column(discord_id, column)
    ...
    async def award_trophies_to_top_players(self):
        ...
        for category in categories:
            top_players = await self.daily_leaderboard_dao.get_top_100_daily_players_by_column(category)
