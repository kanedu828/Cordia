...
class MarketService:
    def __init__(self, market_item_dao: MarketItemDao, item_service: ItemService, player_service: PlayerService):
    ...
    async def buy_market_item(self, market_item_id: int, discord_id: int) -> MarketItem:
        ...
        await self.item_service.insert_item(discord_id, market_item.item_name, market_item.count)
        
        TAX_RATE = 0.05
        tax_adjusted_price = math.ceil(market_item.price - market_item.price * TAX_RATE)
        await self.player_service.increment_gold(market_item.discord_id, tax_adjusted_price)
        
        await self.market_item_dao.delete_market_item(market_item_id)
        return market_item

    async def list_market_item(self, discord_id: int, item_name: str, price: int, count: int) -> MarketItem:
        ...
        await self.item_service.insert_item(discord_id, item_name, -count) # Remove item from seller inventory
        return await self.market_item_dao.insert_market_item(discord_id, item_name, price, count)

    async def unlist_market_item(self, market_item_id: int) -> None:
        """Add a new item to the market."""
        market_item = await self.market_item_dao.get_market_item_by_id(market_item_id)
        await self.item_service.insert_item(market_item.discord_id, market_item.item_name, market_item.count)
        await self.market_item_dao.delete_market_item(market_item_id)
